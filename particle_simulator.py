# -*- coding: utf-8 -*-
"""
Created on Wed Jan  5 22:28:47 2022
...
def main():
 ...
 #   r0 = [0]
    ...
    # axial position, m
    z1 = data[:,0]
    sub_dist = max(z1) # substrate total distance COMSOL
    ...
    # duplicate data for opposite side
    r2 = -data[:,1]
    # axial position, m
    z2 = data[:,0]
    # radial gas velocity, m/s
    ...
    # radial gas velocity, m/s
    vgr = np.concatenate((vgr1,vgr2), axis=0)
    # axial gas velocity, m/s
    vgz = np.concatenate((vgz1,vgz2), axis=0)
    # density, kg/m^3
    rho = np.concatenate((rho1,rho2), axis=0)
    # gas dynamic viscosity, Pa*s
    eta = np.concatenate((eta1,eta2), axis=0)
    # gas mach number
    Ma_com = np.concatenate((Ma_com1,Ma_com2), axis=0)
    # temperature, K
    T_com = np.concatenate((T_com1,T_com2), axis=0)
    ...
    # for x in check:
    #     if results_array[4,x] > (results_array[2,x]+0.00569504)/2.747479187845152:
    ...
    coord = [[-.081,0],[-.081,.005461],[-.0710057,.00075],[0,.0024384],[0,.02],[0.004,.02],[0.004,-.02],[0,-.02],[0,-.0024384],[-.0710057,-.00075],[-.081,-.005461]]
    ...
    plt.plot(dr, dz, 'r-', markersize=1) #r,-z,'o',
...
Dnp_entry.insert(0,'20000') # default nanoparticle default diameter
...
z0_entry.insert(0,'-.08') # default axial starting position
...
Gas_Var.set(GasOptions[1]) # default gas option
